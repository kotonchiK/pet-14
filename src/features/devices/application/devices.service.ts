...
export class DevicesService {
  ...
  async deleteDevice(requestData:DeleteDeviceModel):Promise<void> {
    ...
    if(requestData.userId !== device.userId) throw new ForbiddenException('Device does not belong to the owner')

    await this.devicesRepository.deleteById(requestData.requestId)
  }

  async deleteAllDevices(userId:string, devicesId:string):Promise<void>{
